...
import { commitBlueprintDraft, deleteBlueprintDraft, saveBlueprintDraft } from "@/services/workbench-store";
import type { HistorySelection } from "@/domain/blueprint-history";
...
export function useBlueprintDraft(id: string, state: WorkbenchState) {
  ...
  function create(baseRevision: number, baseBlueprintRevision: number, history?: HistorySelection) {
    let firstSaved = false;
    const session = new BlueprintDraftSession(id, baseRevision, baseBlueprintRevision, history ? async (draft, expected) => {
      const saved = await saveBlueprintDraft(id, draft, expected, firstSaved ? undefined : history);
      firstSaved = true; return saved;
    } : undefined);
  ...
  async function restoreData(data: BlueprintData, baseRevision: number, baseBlueprintRevision: number, history?: HistorySelection) {
    if (!mounted.current) throw new Error("已离开蓝图编辑页，请返回后重新选择要恢复的内容。");
    if (sessionRef.current) throw new Error("请先提交或放弃当前编辑，再恢复另一份草稿。");
    const session = create(baseRevision, baseBlueprintRevision, history);
    session.change(data); local.change(data); await run(session, () => session.writer.flush());
  }
  async function restore(draft: BlueprintDraft, baseRevision = draft.baseRevision, baseBlueprintRevision = draft.baseBlueprintRevision) {
    return restoreData(draft.data, baseRevision, baseBlueprintRevision);
  }
  ...
  useEffect(() => {
    ...
    return () => { mounted.current = false; sessionRef.current?.detach(); };
  }, [id]);
  return { draft: local.draft, activeId, status, change, restore, restoreData, commit, discard, leave, saveAsNew, flush };
}
