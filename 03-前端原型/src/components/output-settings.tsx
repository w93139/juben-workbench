...
import { outputPath, outputSettingsInputSchema, type OutputSettingsInput, type OutputStage } from "@/domain/output-settings";
...
export function OutputSettingsPanel({ project, stage }: { project: Project; stage: OutputStage }) {
  ...
  function persistOutput(revision: number, input: OutputSettingsInput) {
    return project.readOnly
      ...
      : service.saveOutputSettings(project.id, revision, input);
  }
  async function pickDirectory() {
    ...
    try {
      ...
      action.mutate((revision) => persistOutput(revision, { ...next, stage }), { onSuccess: complete });
    } catch (error) {
  ...
  return <section className="output-location" aria-label="输出储存位置">
    ...
    {pendingDirectory && !action.isPending && <div className="folder-retry"><p className="field-hint">待保存：{draft.directory?.name}。原输出位置未改变。</p><Button variant="outline" size="sm" disabled={busy || !valid.success} onClick={() => { if (valid.success) action.mutate((revision) => persistOutput(revision, valid.data), { onSuccess: complete }); }}>重试保存文件夹</Button></div>}
    {action.feedback}
    <details className="folder-fallback" open={manualOpen} onToggle={(event) => setManualOpen(event.currentTarget.open)}><summary>无法选择？备用方式</summary>
      <p className="field-hint">浏览器无法选择目录时，可粘贴完整路径。Mac 访达选中文件夹后，按 Option + Command + C 复制。不同浏览器的项目不会自动共享。</p>
      <form onSubmit={(event) => { event.preventDefault(); if (!busy && valid.success) action.mutate((revision) => persistOutput(revision, valid.data), { onSuccess: complete }); }}>
