...
export function StageContent({ stage, content, project }: { stage: WorkflowStage; content: DemoContent | null; project: Project }) {
  ...
  return <>
    <div className="stage-callout"><strong>{stage.purpose}</strong><br />本阶段目标：{stage.outcome}<br /><span>目前开放档案浏览。操作能力将在开发阶段 {stage.nextPhase} 补齐。</span></div>
    <section className="panel"><div className="panel-title"><h2>{stage.name}档案</h2><span>{content ? "原始样例 · 只读" : "尚未准备"}</span></div>
      {!content ? <div className="empty-state"><FileText size={26} className="mx-auto" /><h2>这部分内容还未建立</h2><p>先在项目总览记录备注与决定。后续将开放{stage.name}的具体操作。</p></div> : <>
        {stage.id === "blueprint" && <Blueprint content={content} />}
        {stage.id === "generation" && <><p className="story-premise">原样例开本包 {content.kitVersion} 已包含以下材料。当前展示现有文件状态，没有调用模型重新生成。</p><div className="record-list">{content.deliverables.map((d) => <div className="record-item" key={d.label}><Check size={15} /><p>{d.label}</p><span className="tag ml-auto">{d.complete ? "原包已具备" : "待补齐"}</span></div>)}</div><p className="field-hint">后续开放分模块模拟生成、重新生成及版本差异查看。</p></>}
        {stage.id === "review" && <><p className="story-premise">右侧分别记录 AI 结构审查、静态一致性检查、模拟测试与真人试玩。AI 结论来自历史蓝图审查；静态结果来自原开本包，不能外推为当前副本或真人体验已通过。</p>{content.documents.filter((d) => d.id === "review").map((doc) => <SourcePreview key={doc.id} document={doc} />)}<p className="field-hint">后续开放问题严重程度、证据位置、修改建议与处理状态。</p></>}
        {stage.id === "playtest" && <><span className="tag amber">尚未真人试玩</span><h3 className="serif text-lg mt-5">纸面之外，还需要一桌玩家。</h3><p className="story-premise mt-4">此项目没有真实试玩记录。计划时长与角色参与度均不能作为已验证的体验结论。</p><div className="record-grid">{["实际时长与各轮卡点", "每个角色的参与和选择", "线索购买与推理结果", "主持执行问题与修改建议"].map((item) => <div className="record-card" key={item}><p>{item}</p><span className="tag neutral mt-3">待记录</span></div>)}</div></>}
        {stage.id === "export" && <><p className="story-premise">原始开本包 {content.kitVersion} 的使用说明可只读查看。导出与玩家材料隔离将在后续阶段实现，当前没有生成新的下载文件。</p>{content.documents.filter((d) => d.id === "kit").map((doc) => <SourcePreview key={doc.id} document={doc} />)}<div className="stage-callout mt-5"><strong>作者资料含谜底</strong><br />未来导出将区分玩家包与主持包，并明确标注审查范围及真人试玩状态。</div></>}
      </>}
    </section>
  </>;
}
