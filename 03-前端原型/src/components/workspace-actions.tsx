...
export function WorkspaceActions({ project, content, stageId }: { project: Project; content: DemoContent | null; stageId?: StageId }) {
  ...
  const resume = workflowStep(project.production ? "generation" : project.blueprint ? "blueprint" : researchStep(project.research));
    ...
    <div className="workspace-action-buttons">
      ...
      <Dialog>
        ...
        <DialogContent className="workspace-action-dialog">
          ...
          {project.blueprint && <p className="field-hint">当前蓝图草稿修订 {project.blueprint.revision}，保留 {project.blueprint.versions.length} 个版本。本项目有 {project.production?.reviews.length ?? 0} 次模拟审查；未调用真实模型，尚未真人试玩。请在“生成与检查”查看对应版本与待复查状态。下方是原始样例记录，不代表当前修改已通过。</p>}
