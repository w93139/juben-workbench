...
function BackupSummary({ backup }: { backup: ProjectBackup }) {
  ...
  return <div className="space-y-3"><p>项目：{backup.project.title} · 备份时间：{new Date(backup.exportedAt).toLocaleString()}</p><p>材料 {state.documents.length} 份（排除 {state.documents.filter(d => d.excluded).length} 份） · 蓝图历史 {state.versions.length} 份 · 草稿 {state.blueprintDrafts.length} 份</p><p>正文材料 {state.review?.artifacts.length ?? 0} 份 · 当前审查报告 {Object.keys(state.review?.reports ?? {}).length} 份 · 历史审查 {state.reviewArchives.length} 份</p><p>包含原有项目记录、已保存的拆解、创作要求及方向选择。</p>{!backup.authoringRecordExists && <p role="status">原项目没有找到当前创作正文记录；本备份包含项目索引与旧版记录，请核对内容是否齐全。</p>}<p className="field-hint">仅包含已保存的浏览器创作数据。原始磁盘文件、费用账本、模型配置和文件夹权限需分别保管。</p></div>;
}
