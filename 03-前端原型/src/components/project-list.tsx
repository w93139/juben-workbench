...
function ProjectTable({ projects }: { projects: Project[] }) {
  return <div className="table-wrap"><table className="project-table"><thead><tr><th>项目名称</th><th>当前阶段</th><th className="table-secondary">参考材料</th><th className="table-secondary">检查记录</th><th className="table-secondary">最近修改</th><th><span className="sr-only">打开</span></th></tr></thead><tbody>{projects.map((project) => <tr key={project.id}><td><Link href={`/projects/${project.id}`} className="project-name"><span className="project-icon"><FileText size={17} /></span><span><strong>{project.title}</strong><small>本机项目 · 独立保存</small></span></Link></td><td><span className="tag neutral">{workflowStep(currentStage(project)).name}</span></td><td className="table-secondary">{project.research.documents.length ? `${project.research.documents.length}份已登记` : "尚未导入"}</td><td className="table-secondary"><span className="table-status">{project.production?.reviews.length ? "有模拟检查记录" : project.template === "names-beyond" ? "有历史检查记录" : "尚未检查"}</span></td><td className="table-secondary"><span className="table-date">{formatDate(project.updatedAt)}</span></td><td><Link href={`/projects/${project.id}`} aria-label={`打开${project.title}`} className="text-link"><ArrowUpRight size={15} /></Link></td></tr>)}</tbody></table></div>;
}
...
export function ProjectListPage({ home = false }: { home?: boolean }) {
  ...
  return <AppFrame title={home ? "工作台首页" : "我的项目"}>
    ...
    <div className="page-footnote"><span><span className="local-dot" />项目保存在此浏览器，清除网站数据后将丢失。</span><span>材料登记 ≠ 内容识别或体验验证</span></div>
  </AppFrame>;
}
