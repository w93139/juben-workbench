...
export function Studio({ project, step }: { project: Project; step: number }) {
  ...
  return <>
    ...
    <footer className="studio-actions"><div>{!connected && <small>{capability.data?.message || (capability.error ? "模型连接状态读取失败" : "正在读取模型连接状态…")}</small>}{dirty && <small>请先保存蓝图调整</small>}{reading && <small>正在读取文件…</small>}</div>
