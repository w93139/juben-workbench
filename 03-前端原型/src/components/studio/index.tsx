...
export function Studio({ project, step }: { project: Project; step: number }) {
  ...
  const reviewReady = capability.data?.reviewReady === true || (capability.data?.reviewReady == null && legacyReady);
  const stepReady = step >= 3 ? reviewReady : analyzeReady;
  ...
  return <>
    ...
    <footer className="studio-actions"><div className="studio-connection-actions"><Button variant="outline" onClick={() => setBudgetOpen(true)}>创作预算</Button><div><small>{budget.error ? "预算读取失败" : budget.data ? `剩余 ${yuan(budget.data.remainingFen)}${budget.data.uncertainCalls ? " · 有待核对费用" : ""}` : "尚未设置创作额度"}</small>{!stepReady && <small>{capability.data?.message || (capability.error ? "模型连接状态读取失败" : "正在读取模型连接状态…")}</small>}{longSource && <small>长剧本将分批读取，按实际模型调用计费。</small>}{requirementsPending && <small>请先保存创作要求</small>}{dirty && <small>请先保存蓝图调整</small>}{reading && <small>正在读取文件…</small>}</div></div>
      ...
      {step === 2 && (canAnalyzeHere ? <Button disabled={locked || requirementsPending || !analyzeReady} onClick={() => void start("analyze", "analysis")}>{state.error ? "重试拆解" : "拆解大纲"}<ArrowRight size={16} /></Button> : <Button disabled={locked || requirementsPending || !analysisCurrent(state) || !state.choiceId || !analyzeReady} onClick={() => void start("blueprint", "blueprint")}>生成蓝图<ArrowRight size={16} /></Button>)}
      {step === 3 && <Button disabled={locked || dirty || !blueprintCurrent(state) || !reviewReady} onClick={() => void start("review", "generation")}>开始交叉验证<ArrowRight size={16} /></Button>}
      {step === 4 && (reviewCurrent(state) ? <Button disabled={locked} onClick={() => void exportKit()}>导出完整档案<Download size={16} /></Button> : <div className="flex flex-wrap gap-2">{!state.job && state.reviewProgress?.checkpoint.runId && state.reviewProgress.blueprintRevision === state.blueprintRevision && blueprintCurrent(state) && <Button disabled={locked || !reviewReady} onClick={() => void start("review", "generation")}>查看继续费用<ArrowRight size={16} /></Button>}<Button variant="outline" disabled={locked} onClick={() => router.push(base + "blueprint")}>{state.job ? "正在交叉验证…" : "返回修改蓝图"}<ArrowRight size={16} /></Button></div>)}
    </footer>
  </>;
}
