...
export function Studio({ project, step }: { project: Project; step: number }) {
  ...
  return <>
    ...
    <Dialog open={!!costPreview} onOpenChange={open => { if (!open && !busy) setCostPreview(null); }}><DialogContent><DialogTitle>本次创作费用</DialogTitle><DialogDescription>确认后开始当前步骤；刷新页面、查询预算和取消此窗口都不会生成内容。</DialogDescription>{costPreview && <>
      ...
      {costPreview.data.checkpoint && <p role="status">{costPreview.data.checkpoint.savedUnits > 0 ? `本次可复用 ${costPreview.data.checkpoint.savedUnits}/7 个已保存阶段。上述金额仍按完整七次保守计算，不是剩余调用的精确费用。` : state.reviewProgress?.checkpoint.runId ? "当前蓝图或模型配置没有可复用的已保存阶段，本次将从头执行。请先备份需要保留的旧成果。" : "本次从蓝图检查开始，完成的阶段会逐项保存。"}{costPreview.data.checkpoint.interruptedUnits > 0 && `有 ${costPreview.data.checkpoint.interruptedUnits} 个阶段结果尚未保存，之前可能已收费；继续会重新调用未保存项。`}</p>}
      ...
      <Button disabled={busy || locked || costPreview.data.budget.remainingFen <= 0 && costPreview.data.checkpoint?.savedUnits !== 7} onClick={() => void confirmStart()}>按项目额度开始</Button><Button variant="outline" disabled={busy} onClick={() => setCostPreview(null)}>取消</Button>
