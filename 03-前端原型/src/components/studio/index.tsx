...
export function Studio({ project, step }: { project: Project; step: number }) {
  ...
  const longSource = step === 1 && !state.job && new TextEncoder().encode(JSON.stringify({ documents: state.documents.filter(d => !d.excluded).map(({ id, name, text }) => ({ id, name, text })), instructions: state.instructions })).length > SINGLE_CONTEXT_BYTES;
