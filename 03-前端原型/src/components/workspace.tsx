...
import { ProjectBackupButton } from "./project-backup";
...
export function WorkspacePage({ projectId, stageId }: { projectId: string; stageId?: StageId }) {
  ...
  return <AppFrame title={step.name} projectId={projectId} workspace>
    {query.isPending ? <Loading /> : !project ? <LoadError error={query.error} retry={() => void query.refetch()} /> : project.readOnly ? <section className="panel"><h1>历史样例</h1><p>原始档案保持只读。请通过左侧加号上传你要改写的剧本。</p></section> : <>
      <div className="workspace-fixed-header" aria-label="当前项目与创作流程"><header className="workspace-header"><div className="project-title-row"><h1 className="serif" title={project.title}>{project.title}</h1><EditProject project={project} label="改名" iconOnly /><ProjectBackupButton key={project.id} project={project} /></div></header>
