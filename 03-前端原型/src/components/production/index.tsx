...
export function ProductionWorkbench({ project, content, reviewFirst = false }: { project: Project; content: DemoContent | null; reviewFirst?: boolean }) {
  ...
  return <div className="main-stack">
    <section className="panel" id="blueprint-review-ready" aria-label="生成与检查准备">
      ...
      {project.readOnly ? <p className="stage-callout mt-4">原始样例保留只读。<Link className="inline-link" href="/projects/new?template=demo">创建演示副本</Link>，在“设计故事”载入蓝图并建立版本后体验。</p> : !version ? <p className="stage-callout mt-4">请先<Link className="inline-link" href={`/projects/${project.id}/stages/blueprint`}>进入设计故事</Link>，保存蓝图并建立一个版本。不必填完所有内容才能练习。</p> : <>
