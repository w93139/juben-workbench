...
export function EditProject({ project, label = "编辑项目" }: { project: Project; label?: string }) {
  ...
  const update = useMutation({ mutationFn: (input: UpdateProjectInput) => service.update(project.id, revision, input), onSuccess: async (next) => { await onSave(next); setOpen(false); setSaved(true); } });
