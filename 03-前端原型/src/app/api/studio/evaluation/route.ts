...
const actionSchema = z.object({ action: z.enum(["discover", "start", "resume", "prepare", "cancel"]) }).strict();
...
export async function POST(request: Request) {
  try {
    ...
    if (parsed.data.action === "prepare") return localJson(await modelEvaluationEngine.prepareRevised());
