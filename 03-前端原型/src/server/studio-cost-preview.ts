...
import { analysisSelectionSchema, estimateAnalysisPlan, sourceSelectionSchema } from "./long-analysis";
...
import { calculateQuotedFen, reserveQuotedFen } from "./studio-pricing";
...
export async function previewStudioCost(billing: StudioBilling, config: StudioConfig, projectId: string, operation: StudioOperation, input: unknown, production?: StudioProductionStore) {
  ...
  const models = operation === "review" ? [config.mainModel, config.reviewA, config.reviewB] : operation === "analyze" ? [config.analysisModel || config.mainModel] : [config.mainModel];
  ...
  const analysisPlan = operation === "analyze" && inputBytes > ANALYSIS_DIRECT_BYTES ? estimateAnalysisPlan(studioInputs.analyze.parse(input).documents) : undefined;
  const callsMax = operation === "review" ? plan!.targets.length + (scopes === undefined ? 6 : 1 + scopes * 5) : analysisPlan ? analysisPlan.length : 1;
  let contextLimit = operation === "review" ? SINGLE_CONTEXT_BYTES : inputBytes;
  if (analysisPlan) contextLimit = ANALYSIS_CALL_BYTES;
  ...
  const estimateFen = analysisPlan
    ? analysisPlan.reduce((sum, call) => sum + calculateQuotedFen(quotes[0], Math.ceil((call.inputBytes + 2048) * 1.2), call.maxTokens), 0)
    : operation === "review" ? perModel[0] * (plan!.targets.length + (scopes === undefined ? 2 : 1 + scopes)) + perModel[1] * (scopes === undefined ? 2 : scopes * 2) + perModel[2] * (scopes === undefined ? 2 : scopes * 2) : perModel[0] * callsMax;
