...
import { studioInputs, studioAnalysisSchema, studioAuditSchema, studioArtifactSchema, type StudioOperation } from "@/domain/studio";
...
export async function previewStudioCost(billing: StudioBilling, config: StudioConfig, projectId: string, operation: StudioOperation, input: unknown, production?: StudioProductionStore) {
  ...
  let callsMax = operation === "review" ? plan!.targets.length + 6 : 1;
  ...
  const schemas = operation === "analyze" ? [studioAnalysisSchema, sourceSelectionSchema, analysisSelectionSchema]
    : operation === "blueprint" ? [blueprintDataSchema] : [studioAuditSchema, studioArtifactSchema];
  ...
  const estimateFen = operation === "review" ? perModel[0] * (plan!.targets.length + 2) + perModel[1] * 2 + perModel[2] * 2 : perModel[0] * callsMax;
