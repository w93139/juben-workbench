...
import { ANALYSIS_BATCH_BYTES, ANALYSIS_SEGMENT_BYTES, ANALYSIS_MAX_BATCHES, ANALYSIS_CALL_BYTES, ANALYSIS_EXTRACT_TOKENS } from "@/domain/analysis-limits";
...
export async function analyzeLongSource(data: { documents: Document[]; instructions: string }, options: LongAnalysisOptions): Promise<StudioAnalysis> {
  ...
  const selected = await boundedCall("根据全部分段提取并逐层合并的研究摘要形成统一拆解大纲和2至5个原创方向。你看到的是摘要，不得声称自己直接逐字读过全部原文。outline包括真相、因果时间线、人物关系、信息分配、证据链、轮次节奏，区分明确事实、分析推断与待定事项。跨片段矛盾保留为unknowns，方向重建人物、动机、事件因果和线索，不只换名。sourceRefIds只选择本次输入摘要已有的citationId；不输出摘录与位置，程序将精确回填。", { notes: noteInput(notes), instructions: data.instructions }, analysisSelectionSchema, ANALYSIS_EXTRACT_TOKENS);
  const { sourceRefIds, ...content } = selected;
