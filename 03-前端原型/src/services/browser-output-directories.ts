...
// Selecting an output location only stores the exact directory reference; it creates no files.
export class BrowserOutputDirectories implements OutputDirectoryPort {
  ...
  async pick(): Promise<PickedOutputDirectory | null> {
    ...
    try {
      ...
      throw new ServiceError("DIRECTORY_UNAVAILABLE", "未能访问所选文件夹，原设置已保留。请允许访问文件夹后重试。");
    }
