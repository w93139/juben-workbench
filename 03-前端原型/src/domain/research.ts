...
export const directionSchema = z.object({
  players: z.number().int().min(2).max(12), genre: z.string().trim().min(1).max(60), minutes: z.number().int().min(60).max(600),
...
}).refine((d) => d.deduction + d.emotion + d.mechanics === 100, { message: "三种体验比例相加需要等于 100%。", path: ["deduction"] });
...
export const researchSchema = z.object({
  ...
  choices: z.array(mechanismChoiceSchema), direction: directionSchema.nullable(), directionConfirmed: z.boolean(),
  ...
  if (state.directionConfirmed && (!state.direction || state.analysisRevision !== state.materialRevision || !state.choices.some((c) => c.choice !== "omit"))) ctx.addIssue({ code: "custom", message: "原创方向缺少有效的研究依据。" });
});
