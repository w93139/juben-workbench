...
export type EditableBlueprintSection = Exclude<keyof BlueprintData, "premise" | "truth">;
export type EditorField = { key: string; label: string; type?: "text" | "number" | "boolean" | "reference" | "references" | "state"; target?: EditableBlueprintSection; hint?: string };
export const sectionLabels: Record<EditableBlueprintSection, string> = { characters: "角色", relationships: "关系", events: "事件", knowledge: "信息发放", claims: "推理结论", clues: "线索", rounds: "轮次", triggers: "主持触发", endings: "终局" };
export const knowledgeStates = { known: "已知", partial: "知道一部分", hidden: "知道但隐瞒", false: "持有错误认识", unknown: "不知道" };
const ref = (key: string, label: string, target: EditableBlueprintSection): EditorField => ({ key, label, target, type: "reference" });
const refs = (key: string, label: string, target: EditableBlueprintSection, hint?: string): EditorField => ({ key, label, target, type: "references", hint });
export const blueprintFields: Record<EditableBlueprintSection, EditorField[]> = {
...
export function newBlueprintRow(section: EditableBlueprintSection, id: string): BlueprintRow {
...
export function blueprintRowLabel(row: BlueprintRow, section: EditableBlueprintSection, data: BlueprintData): string {
  if (section === "relationships") return [row.fromId, row.toId].map((id) => data.characters.find((c) => c.id === id)?.name || (id ? `已失效：${id}` : "待选角色")).join(" ↔ ");
...
export const sectionLimits: Record<EditableBlueprintSection, number> = { characters: 40, relationships: 200, events: 200, knowledge: 2000, claims: 200, clues: 200, rounds: 100, triggers: 200, endings: 100 };
export function removeBlueprintRow(data: BlueprintData, section: EditableBlueprintSection, id: string): BlueprintData {
  // References intentionally survive deletion, especially private-clue recipients.
  return { ...data, [section]: data[section].filter(row => row.id !== id) };
}
export function affectedReferences(data: BlueprintData, target: EditableBlueprintSection, id: string) {
  const affected: string[] = [];
  for (const section of Object.keys(blueprintFields) as EditableBlueprintSection[]) {
    for (const row of data[section] as BlueprintRow[]) {
      const fields = blueprintFields[section].filter(field => field.target === target && (Array.isArray(row[field.key]) ? (row[field.key] as string[]).includes(id) : row[field.key] === id));
      if (fields.length) affected.push(`${sectionLabels[section]}「${blueprintRowLabel(row, section, data)}」：${fields.map(f => f.label).join("、")}`);
    }
  }
  return affected;
}
