...
async function preview(page: Page, runId: string | null, savedUnits: number) {
  await page.route("**/api/studio/budget**", async route => {
    ...
    return route.fulfill({ json: { projectId: body.projectId, operation: "review", previewId: crypto.randomUUID(), budget: budgetFixture(body.projectId), quotes: testQuotes(), callsMax: 7, estimateFen: 50, checkpoint: { runId, savedUnits, interruptedUnits: savedUnits ? 1 : 0 } } });
  });
}
test("阶段正文和完整报告运行中落盘，失败与刷新保留，手机可读且旧通过不能导出", async ({ page }) => {
  ...
  await expect(region).toContainText("已保存 3/7 阶段"); await region.getByText("独立检查A · 已保存报告", { exact: true }).click(); await expect(region).toContainText("阅读负担需真人试玩"); await region.getByText("角色本 · A的character材料", { exact: true }).click(); await expect(region).toContainText("A选择是否公开档案");
  ...
  await preview(page, view.reviewProgress!.runId, 3); await page.getByRole("button", { name: "查看继续费用" }).click(); await expect(page.getByRole("dialog")).toContainText("本次可复用 3/7"); await page.getByRole("dialog").getByRole("button", { name: "取消", exact: true }).click(); expect(posts).toBe(0);
});
...
test("阶段成果v2完整备份与恢复保留全文，新副本无继续或导出权限且不外呼", async ({ page }) => {
  ...
  expect(backup.schemaVersion).toBe(2); expect(backup.workbench.reviewProgress?.checkpoint.runId).toBeNull(); expect(backup.workbench.reviewProgress?.checkpoint.review.artifacts).toEqual(s.view.reviewProgress!.review.artifacts);
