...
vi.mock("@/server/studio-settings", () => ({ studioSettingsStore: { config: () => null, safe: () => ({ providerConfigured: false, analyzeReady: false, blueprintReady: false, reviewReady: false }) } }));
...
it("未配置模型接口返回503且零外呼，capability仅暴露布尔状态和安全提示", async () => {
  ...
  expect(status.status).toBe(200); const body = await status.json(); expect(body.configured).toBe(false); expect(body.analyzeReady).toBe(false); expect(body.reviewReady).toBe(false); expect(Object.keys(body).sort()).toEqual(["analyzeReady", "blueprintReady", "configured", "message", "providerConfigured", "reviewReady"]);
