...
import { mkdtemp, mkdir, open, readFile, readdir, rm, stat, symlink } from "node:fs/promises";
...
test("主动选择目录直接保存该位置，不创建子文件夹，导出直接写入", async () => {
  const output = join(root, "指定成果位置"); await mkdir(output);
  const selected = await directories.registerChosenDirectory(output);
  expect(selected.name).toBe("指定成果位置"); expect(await readdir(output)).toEqual([]);
  await directories.writeSelectedZip(selected.id, "完整档案.zip", new Uint8Array([1, 2]));
  expect(await readFile(join(output, "完整档案.zip"))).toEqual(Buffer.from([1, 2]));
  expect(await readdir(output)).toEqual(["完整档案.zip"]);
});
